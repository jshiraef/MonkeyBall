/*terrain generation and colliding algorithms from CodeAcademy.cc OpenGL tutorial*/
...
void Terrain::ComputeTriangle(std::vector<Coord> &triangle)
{
	...
	// keep the vertices of the triangle
...
void Terrain::SetNormalPerVertex(int x, float y, int z)
{
...
void Terrain::Load(int level)
{
	...
	fread(&heightmap, TERRAIN_SIZE * TERRAIN_SIZE, 1, pFile); //assigned values ​​in the range 0..255
	fclose(pFile);
...
float Terrain::GetHeight(float x, float z)
{
	...
	else
	{
		...
		if (fracz == 1.0f - fracx) // the intersection between triangles of vertices
		{
			return GetVertexHeigh(intx + 1, intz)*(fracz / sin(45.0f)) + GetVertexHeigh(intx, intz + 1)*(1 - fracz / sin(45.0f));
		}
		if (fracz < 1.0f - fracx) //within the first triangle of vertices
		{
		...
		if (fracz > 1.0f - fracx) //within the second triangle of vertices
		{
...
bool Terrain::IsColliding(Coord P, float radius, Triangle tri, Coord &center)
{
	...
	if (lambda > 1.0f) return false; //if the distance to the plane is not enough collided
	Coord I;  I.x = P.x + lambda*V.x;  I.y = P.y + lambda*V.y;  I.z = P.z + lambda*V.z; //intersection point

																						//check if the point belongs to the triangle intersection
	...
	//At this point we know that collided with the tri triangle
	//Now recalculate the position of the center to leave the field on the surface of tri
...
std::vector<Vector> Terrain::GetCollisionNormals(Coord &center, float radius)
{
	...
	else
	{
		...
		//get the matrix position of the first triangle of vertices in the quad
		trianglex = intx * 2;
		trianglez = intz;
...
bool Terrain::IsColliding(Coord P, Coord Q, Triangle tri, float &lambda)
{
	...
	if (tri.N.x*V.x + tri.N.y*V.y + tri.N.z*V.z == 0.0f)
	{
		return false;
	}
...
float Terrain::GetSegmentIntersectionLambda(float x, float y, float z, float vx, float vy, float vz, float dist)
{
	...
	//get the matrix position of the first triangle of quad vertices which are both ends of the segment
	int trianglePx = ((int)P.x) * 2, trianglePz = (int)P.z;
	int	triangleQx = ((int)Q.x) * 2, triangleQz = (int)Q.z;

	//consider the out of range
	...
	//lamba get the possible intersections of
